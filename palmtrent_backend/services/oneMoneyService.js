...
class OneMoneyService {
  ...
  init() {
    if (!process.env.PAYNOW_INTEGRATION_ID || !process.env.PAYNOW_INTEGRATION_KEY) {
      console.warn('OneMoney Service: Paynow credentials not configured');
      return;
    }
  ...
  /**
   * Format phone number for OneMoney
   */
  ...
  /**
   * Validate OneMoney phone number
   */
  validateOneMoneyNumber(phone) {
    const formatted = this.formatPhoneNumber(phone);
    if (!formatted) return { valid: false, error: 'Phone number is required' };

    // OneMoney (NetOne) numbers start with 071
    const oneMoneyPattern = /^263(71)\d{7}$/;

    if (!oneMoneyPattern.test(formatted)) {
      return {
        valid: false,
        error: 'Invalid OneMoney number. Must start with 071'
      };
    }
  ...
  async initiatePayment(paymentReference, amount, phoneNumber, email) {
    ...
    const validation = this.validateOneMoneyNumber(phoneNumber);
    if (!validation.valid) {
    ...
    console.log('Initiating OneMoney payment:', {
    ...
    try {
      // Send to mobile money (OneMoney)
      const response = await this.paynow.sendMobile(payment, formattedPhone, 'onemoney');

      console.log('OneMoney response:', response);

      if (response.success) {
        return {
          success: true,
          pollUrl: response.pollUrl,
          instructions: response.instructions || 'Please check your phone and enter your OneMoney PIN to confirm the payment',
        ...
        return {
          success: false,
          error: response.error || 'OneMoney payment initiation failed',
          status: 'failed'
        };
      }
    } catch (error) {
      console.error('OneMoney initiation error:', error);
      throw error;
    }
  }
  ...
  async pollStatus(pollUrl) {
    ...
    try {
      ...
      console.log('OneMoney poll status:', status);
      ...
      console.error('OneMoney poll error:', error);
      throw error;
    }
  }
  ...
  /**
   * Process OneMoney payment for a booking
   */
  async processPayment(paymentId) {
    ...
    if (payment.paymentMethod !== 'onemoney') {
      throw new Error('Payment is not a OneMoney payment');
    }

    if (!payment.customer?.phone) {
      throw new Error('Customer phone number is required for OneMoney');
    }
    ...
    if (result.success) {
      ...
      payment.metadata = {
        ...payment.metadata,
        oneMoneyInstructions: result.instructions
      };
      await payment.save();
    } else {
  ...
  async checkAndUpdateStatus(paymentReference) {
    ...
    if (pollResult.paid && payment.status !== 'confirmed') {
      ...
      await paymentService.confirmPayment(paymentReference, {
        ...
        metadata: { oneMoneyPollResult: pollResult }
      });
  ...
  /**
   * Get OneMoney payment instructions
   */
  getPaymentInstructions(phoneNumber) {
    return {
      steps: [
        'You will receive a prompt on your phone',
        'Enter your OneMoney PIN to authorize the payment',
        'Wait for confirmation message',
        'Payment will be confirmed automatically'
      ],
      phone: this.formatPhoneNumber(phoneNumber),
      timeout: '5 minutes',
      support: 'If you do not receive a prompt, please check your OneMoney balance and try again'
    };
  }
}

module.exports = new OneMoneyService();
